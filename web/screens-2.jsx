...
const HakiOverlay = ({ onComplete, volume = 0.9 }) => {
  useEffect(() => {
    const audio = new Audio(
      "http://127.0.0.1:8765/assets/sounds/Monkey%20D%20Luffy%20Haki%20Sound%20%20One%20Piece.mp3"
    );
    audio.volume = volume;
    audio.play().catch(() => {});
...
const DraftScreen = ({
  ...
  // Local spinning animation (cosmetic only — real char comes from Python)
  ...
  const spin = async () => {
    ...
    // Deduplicate, uppercase, exclude the result from candidates
    ...
    // Build 29 reel items cycling through shuffled unique pool
    ...
    <div className={`stage acc-${universe.id}`} data-screen-label="03 Draft">
      ...
      <div style={{
        ...
        <SlotColumn
          ...
          onSlotClick={(posIdx) => handleAssign(posIdx)}
          imgUrl={imgUrl}
        />
        <PositionColumn
          positions={positions}
  ...
  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
    {positions.map((pos, i) => {
      ...
      return (
        ...
        >
          {/* Fixed-size image cell — all portrait frames are identical */}
          <div style={{
...
/* ----- Right action panel ----- */
const ActionPanel = ({ universe, draft, spinning, reelChars, onSpin, onSkip, onUndo, onSwitch, onReady, ready, imgUrl, onMenu, onPlayers, onPass, switchActive, hakiVolume, setHakiVolume }) => {
...
/* ===================================================== */
/* SCREEN 4 — RESULT (ROSTERS LOCKED)                    */
/* ===================================================== */
